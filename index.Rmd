...
Next, we load in the dataset. Please see https://github.com/tylerfarr5/rb-fantasy-football for the full dataset. 
...
We start by looking at a correlation matrix of variables in the model (not including previous season performance variables - that will be done separately). We see some obviously strong correlations, like the percentage of the salary cap and cash incentives, which are both salary information for players. There's also a very strong relationship between a player's Madden rating and their average draft position (ADP). This relationship makes sense, as highly rated players will be drafted higher.
...
res <- cor(corrs.int)
colnames(res) <- c("Age", "Height", "Weight", "Year Pro", "RB % of Sal Cap", "RB Sal Cash Inctv", "OL % of Sal Cap", "OL Sal Cash Inctv", "QB % of Sal Cap", "QB Sal Cash Inctv", "Overall NFL Draft Pos", "Madden Rating", "PPR FF Avg Draft Pos", "Half-PPR Fantasy Pts")
rownames(res) <- c("Age", "Height (in)", "Weight (lbs)", "Year Pro", "RB % of Salary Cap", "RB Salary Cash Incentives", "OL % of Salary Cap", "OL Salary Cash Incentives", "QB % of Salary Cap", "QB Salary Cash Incentives", "Overall NFL Draft Position", "Madden Rating", "PPR FF Average Draft Position", "Half-PPR Fantasy Points")


corrplot::corrplot(res,type = "upper", order = "hclust", tl.col = "black", tl.srt = 55, title = "Correlation Matrix on NFL Running Back Data (2012-2022)", mar=c(0,0,1,0))
```
...
```{r warning= FALSE, message = FALSE, echo = FALSE, include = FALSE}
#Testing Normality of Data - fails
...
#Must remove 7&8 40+ yard runs since only one observation of them
bonf1_train <- train %>%
  group_by(Fantasy_Points_Half_PPR) %>%
  filter(Prev40Runs <6) %>%
  ungroup()

#Since non-parametric, use Bonferroni Correction.
pairwise.t.test(bonf1_train$Fantasy_Points_Half_PPR, factor(bonf1_train$Prev40Runs), p.adj = "bonf")
```

Next, "boom" or "bust" running backs are typically those with 'big-play ability.' Those can be explored by 40+ yard runs in the previous season. The boxplots below show a statistically significant difference in the average half-PPR fantasy points in at least one of the groups. 

Upon further inspection, we see a significant difference between 0 and 3 40+ yard runs. 7 and 8 were not included in post-hoc testing due to there only being a single occurrence in the data. Additionally, since Tukey's HSD test could not be performed here due to non-parametric data, a Bonferroni correction was made to the p-values instead. Since this is generally more conservative, we see only one significant difference.

There is a generally increasing trend in the boxplots as well, so there is likely a relationship between the number of 40+ yard runs and a running back's fantasy production.
...
```{r warning = FALSE, message = FALSE, echo = FALSE, include = FALSE}
ft.players <- train %>%
  filter(FranchiseTag == 1) #within 15, 25, 35

jon_question <- train %>%
  filter(Year == 2012 & PPR_ADP < 46.7)
jon_question2 <- train %>%
  filter(Year == 2017 & PPR_ADP < 37)
jon_question3 <- train %>%
  filter(Year == 2020 & PPR_ADP <50)

jon.q <- rbind(jon_question, jon_question2, jon_question3)

ggplot(data = jon.q, aes(sample = Fantasy_Points_Half_PPR, color = FranchiseTag)) +
  stat_qq()+
  stat_qq_line()

var.test(Fantasy_Points_Half_PPR ~ FranchiseTag, data = jon.q)
t.test(Fantasy_Points_Half_PPR ~ FranchiseTag, data = jon.q, var.equal = TRUE, alternative = 'less')

wilcox.test(Fantasy_Points_Half_PPR ~ FranchiseTag, alternative = 'less', data = jon.q) #sig lower for 0
```

I was curious about this, so I dove a little deeper and filtered the data to show how franchise tagged players perform relative to non-franchise tagged players, but only those drafted within the top 35 picks of the fantasy draft. Interestingly, there is enough evidence to claim a significant difference in the average half-PPR fantasy points for players on and off a franchise tag. As we can see in the plot below, there is clearly a massive jump in the median half-PPR fantasy points for non-franchise-tagged players. 

```{r warning = FALSE, message = FALSE, echo = FALSE}
ggplot(data = jon.q, aes(x = FranchiseTag, y = Fantasy_Points_Half_PPR)) + geom_boxplot(fill = "orange", alpha = 0.2) +
    labs(x = "Franchise Tag", y = "Half-PPR Fantasy Points", title = "Boxplot of Half-PPR Fantasy Points vs. Franchise Tag \n(2012-2022): [+/- 35 picks PPR ADP]") +
  theme_classic() +
  coord_flip() +
  theme(plot.title = element_text(hjust = 0.5))
```
...
pairwise.t.test(train$Fantasy_Points_Half_PPR, factor(train$DraftRound), p.adj = "bonf")
```

In this final boxplot, we examine what round a running back was drafted in the NFL draft and their corresponding average half-PPR fantasy points. There is an expected generally decreasing trend as higher-scoring players are typically starters, whom are taken in the higher rounds.
...
pairwise.t.test(train2$Fantasy_Points_Half_PPR, factor(train2$DraftRound), p.adj = "bonf")
```
...
```{r warning = FALSE, message = FALSE, echo = FALSE}
Model.Test.Non_Rookies = c("Random Forest", "Gradient Boost", "PCR, 5 comp", "PLS, 2 comp", "Linear Regression (R-sq)", "Linear Regression (AIC)")
...
```{r warning = FALSE, message = FALSE, echo = FALSE}
Model.Test.Rookies = c("Linear Regression", "PCR, 1 comp", "PLS, 1 comp", "Random Forest", "Gradient Boost", "Decision Tree")
